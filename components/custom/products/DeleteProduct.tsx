...
function DeleteProduct({
  ...
  // Check if entered text matches product name
  useEffect(() => {
    if (productName) {
      setIsValid(confirmText.trim() === productName);
    }
  }, [confirmText, productName]);
      ...
      <DialogContent className="sm:max-w-[400px] p-5">
          ...
          <DialogDescription className="text-sm text-gray-500">
            ...
            {productSku && (
              <span className="block mt-1 text-xs text-gray-400">
                SKU: <span className="font-mono">{productSku}</span>
              </span>
            )}
          </DialogDescription>
        </DialogHeader>
        ...
        <div className="bg-red-50 border border-red-200 rounded-md p-2.5 my-1">
          <p className="text-xs text-red-700 flex items-center gap-1.5">
            ...
            This will permanently delete this product and remove it from your inventory.
          </p>
        </div>
        ...
        <div className="space-y-1.5">
          <Label htmlFor="confirm-delete" className="text-xs font-medium text-gray-700">
            Type <span className="font-bold text-gray-900">{productName}</span> to confirm
          </Label>
          <Input
            id="confirm-delete"
            type="text"
          ...
          {confirmText && !isValid && (
            <p className="text-xs text-red-500">
              Please type the product name correctly
            </p>
          )}
          {isValid && (
