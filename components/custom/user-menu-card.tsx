...
export function UserMenuCard({ onNavigate, onSignOut, user, vendor }: UserMenuCardProps) {
  ...
  useEffect(() => {
    const getAuthUserAvatar = async () => {
      try {
        const { data: { user: authUser } } = await supabase.auth.getUser();
        if (authUser?.user_metadata?.avatar_url) {
          setAuthAvatarUrl(authUser.user_metadata.avatar_url);
        }
      } catch (error) {
        console.error("Error fetching auth user avatar:", error);
      }
    };
      ...
      <div className="p-4">
        <div className="flex items-center gap-3">
          <Avatar className="h-12 w-12 border-2 border-primary/10">
            <AvatarImage src={user?.profile_image || authAvatarUrl || undefined} />
